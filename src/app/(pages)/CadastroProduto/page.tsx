        ...
        <main className="min-h-screen bg-[#F8F9FA] text-black flex items-center justify-center p-5 font-sans text-sm">
            <div className="bg-white shadow-2xl rounded-2xl p-8 w-full max-w-lg relative border-t-8 border-teal-600">
                ...
                <div className="text-center mb-8 mt-10">
                    <h1 className="text-3xl font-extrabold text-gray-800">Novo Produto</h1>
                    <p className="text-gray-500">Preencha os dados para o catálogo</p>
                </div>

                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold uppercase text-gray-400 ml-1">ID Identificador</label>
                        <input required type="text" name="id" value={produto.id} onChange={handleChange} className={inputStyle} placeholder="Ex: PROD-001" />
                    </div>

                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold uppercase text-gray-400 ml-1">Nome do Item</label>
                        <input required type="text" name="nome" value={produto.nome} onChange={handleChange} className={inputStyle} placeholder="Ex: Tênis Esportivo" />
                    </div>

                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold uppercase text-gray-400 ml-1">Descrição Curta</label>
                        <textarea required name="desc" value={produto.desc} onChange={handleChange} className={`${inputStyle} h-24 resize-none`} placeholder="Conte mais sobre o produto..." />
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold uppercase text-gray-400 ml-1">Foto do Produto</label>
                    ...
                    <button type="submit" className="w-full bg-teal-600 text-white py-3 rounded-xl font-bold text-lg hover:bg-teal-700 shadow-lg shadow-teal-100 transition-all active:scale-95 mt-4">
                        Salvar Produto
                    </button>
                </form>
            </div>
